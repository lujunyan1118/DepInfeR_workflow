...
```{r GDSC_diffCoef_box,fig.height=8, fig.width=15}
...
ggplot(plotTab, aes(x = target, y = coef, group=Cancer_Type)) +
 geom_jitter(
  ...
  ggtitle("Protein Dependence associated with Cancer Type (p.adj < 0.05, FC > 0.1)") + ylab("Protein Dependence Coefficient") + xlab("Protein") + theme_bw() + 
...
```{r GDSC_diffCoef_radar, fig.height=5, fig.width=7}
...
```{r gdsc_pval_vis, fig.height=6, fig.width=12}
...
p <- ggplot(data=plotTab, aes(x= mutName, y=-log10(p.adj),
  ...
  theme_bw() + scale_color_manual(values = colList2) + 
  scale_y_continuous(trans = "exp", limits = c(0,2.5), breaks = c(0,1,1.5,2)) + 
  theme_custom +
  annotate(geom = "text", x = 0.5, y = -log10(0.1) + 0.25, label = "FDR = 0.1") +
...
```{r gdsc boxplot, fig.height=5, fig.width=5}
